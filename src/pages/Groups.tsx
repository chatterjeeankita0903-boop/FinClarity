...
import { useGroups, useAddGroup, useDeleteGroup, useTransactions, useUpdateTransaction } from '@/hooks/useSupabaseData';
...
const Groups = () => {
  ...
  const handleCreate = () => {
    ...
    addGroup.mutate({
      ...
      onSuccess: () => { setNewName(''); setNewMembers(['']); setShowCreate(false); },
    });
  };
  ...
  const summary = useMemo(() => {
    let owedToYou = 0, settledCount = 0;
    Object.values(groupData).forEach(({ balances }) => {
      balances.forEach(m => { if (m.settled && m.amount > 0) settledCount++; else if (m.amount > 0) owedToYou += m.amount; });
    });
                  ...
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                    <div className="px-4 pb-4 space-y-2">
                      ...
                      <div className="flex items-center gap-2 pt-2">
                        ...
                        <button onClick={() => deleteGroup.mutate(g.id)} className="p-2.5 rounded-xl bg-secondary text-muted-foreground hover:text-destructive transition-colors">
          ...
          <div className="sheet-overlay" onClick={() => setShowCreate(false)}>
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="sheet-panel" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="space-y-2 mb-4">
                  {newMembers.map((m, i) => (
                    <div key={i} className="flex gap-2">
                      <input value={m} onChange={(e) => { const n = [...newMembers]; n[i] = e.target.value; setNewMembers(n); }}
                        placeholder="Member name" className="flex-1 bg-secondary rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground outline-none border border-border focus:border-primary" />
                      {newMembers.length > 1 && (
              ...
              <div className="sheet-footer">
                <button onClick={handleCreate} disabled={addGroup.isPending} className="w-full min-h-12 gradient-primary text-primary-foreground font-semibold py-3 rounded-xl disabled:opacity-50">
