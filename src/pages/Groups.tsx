...
import { useTransactions, useGroups, useSettlements } from '@/hooks/useSupabaseData';
...
const Groups = () => {
  ...
  const groupData = useMemo(() => {
    ...
    groups.forEach(g => {
      ...
      let myExpenses = 0;
      g.members.forEach(m => { memberMap[m.name] = { total: 0, settled: true }; memberExpenses[m.name] = 0; });
      groupTxns.forEach(t => {
      ...
      data[g.id] = {
        balances: g.members.map(m => ({ id: m.id, name: m.name, amount: memberMap[m.name]?.total || 0, settled: memberMap[m.name]?.settled ?? true })),
        memberExpenses,
        myExpenses,
      };
    });
  ...
  const handleSettleUp = async (groupId: string, members: MemberBalance[]) => {
    const unsettled = members.filter(m => !m.settled && m.amount > 0);
    const memberNames = unsettled.map(m => m.name.toLowerCase());
    // Find transaction IDs relevant to these members
    const relevantTxIds = transactions
      .filter(t => t.groupId === groupId || t.splits.some(s => memberNames.includes(s.name.toLowerCase())))
      .map(t => t.id);

    try {
      for (const m of unsettled) {
        await settleUp({ groupId, amount: m.amount, transactionIds: relevantTxIds, memberName: m.name });
      }
        ...
        {groups.map(g => {
          ...
          return (
            <motion.div key={g.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border rounded-xl overflow-hidden">
              <button onClick={() => setExpandedGroup(isExpanded ? null : g.id)} className="w-full flex items-center justify-between p-4 text-left">
                  ...
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                    <div className="px-4 pb-4 space-y-2">
                      {members.map(m => (
                        <div key={m.id} className="flex items-center justify-between py-2 px-3 rounded-lg bg-secondary/50">
                          ...
                          <div className="flex items-center gap-2">
                            {m.settled ? (
                              <span className="text-xs text-muted-foreground italic flex items-center gap-1"><Check className="w-3 h-3" /> settled</span>
                            ) : m.amount > 0 ? (
                        ...
                        <div className="space-y-1.5">
                          ...
                          {members.map(m => (
                            <div key={m.id + '-exp'} className="flex items-center justify-between py-1.5 px-3 rounded-lg bg-secondary/30">
                              <div className="flex items-center gap-2">
                                <div className="w-6 h-6 rounded-full bg-accent/20 flex items-center justify-center text-[10px] font-bold text-accent">{m.name.charAt(0).toUpperCase()}</div>
                      ...
                      <div className="flex items-center gap-2 pt-2">
                        {netBalance > 0 ? (
                          <button onClick={() => handleSettleUp(g.id, members)} className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-primary/10 border border-primary/30 text-primary text-sm font-semibold transition-colors hover:bg-primary/20">
                            💸 Settle Up — {formatAmount(netBalance)}
                          </button>
                        ) : netBalance < 0 ? (
                          <button onClick={() => handleSettleUp(g.id, members)} className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-destructive/10 border border-destructive/30 text-destructive text-sm font-semibold transition-colors hover:bg-destructive/20">
                            💸 Pay {formatAmount(netBalance)}
                          </button>
                        ) : null}
                        <button onClick={() => deleteGroup(g.id)} className="p-2.5 rounded-xl bg-secondary text-muted-foreground hover:text-destructive transition-colors">
                ...
                <div className="space-y-2 mb-4">
                  {newMembers.map((m, i) => (
                    <div key={i} className="flex gap-2">
                      <input value={m} onChange={(e) => { const n = [...newMembers]; n[i] = e.target.value; setNewMembers(n); }} placeholder="Member name" className="flex-1 bg-secondary rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground outline-none border border-border focus:border-primary" />
                      {newMembers.length > 1 && (
